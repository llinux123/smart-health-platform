...
interface DoctorDetail extends DoctorInfo {
  id: number
  deptName: string
}
...
const doctorInfoMap: Record<number, DoctorInfo> = {
...
export function mockGetDoctorDetail(id: number): Promise<DoctorDetail> {
  const doctor = doctorInfoMap[id]
  if (!doctor) return Promise.reject(new Error('医生不存在'))
  return Promise.resolve({ id, ...doctor, deptName: '' })
}

export function mockListSchedules(params?: { deptName?: string; workDate?: string }): Promise<ScheduleItem[]> {
...
export function mockSeckill(_data: any): Promise<SeckillResult> {
...
export function mockGetOrderDetail(): Promise<OrderItem> {
  return Promise.resolve({
    id: 1,
...
export function mockListOrders(): Promise<OrderItem[]> {
  return Promise.resolve([
    {
      id: 1,
      orderSn: 'REG_20260630_0001',
...
export function mockCancelOrder(_orderSn: string): Promise<void> {
  return Promise.resolve()
}

export function mockPayOrder(_orderSn: string): Promise<void> {
  return Promise.resolve()
}
