...
export function mockMultimodalAnalyze(): Promise<AnalyzeResult> {
...
export function mockCreateSession(): Promise<string> {
  return Promise.resolve('SESSION_' + Date.now())
}

export function mockListSessions(): Promise<SessionItem[]> {
  return Promise.resolve([
    {
      id: 1,
...
export function mockGetSessionHistory(): Promise<ChatMessage[]> {
  return Promise.resolve([
    {
      role: 'user',
